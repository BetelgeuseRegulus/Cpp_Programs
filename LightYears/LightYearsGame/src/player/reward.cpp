...
{
    Reward::Reward(World *world,
                   const std::string &texturePath,
                   RewardFunc rewardRunc, float speed)
        : Actor{world, texturePath},
          rewardFunc_{rewardRunc},
          speed_{speed}
    {
    }
    ...
    void Reward::Tick(float deltaTime)
    {
        ...
        AddActorLocationOffset({0.f, speed_ * deltaTime});
    }
    ...
    void Reward::OnActorBeginOverlap(Actor *otherActor)
    {
        ...
        // 奖励物品与飞船碰撞,飞船获得该奖励
        // 通过检测 object 的 Id 而不是指针(地址)提高效率
        // if (playerSpaceship.lock().get() == otherActor)
        if (playerSpaceship.lock()->GetUniqueID() == otherActor->GetUniqueID())
        {
            rewardFunc_(playerSpaceship.lock().get());
            Destroy();
        }
    }
    ...
    void RewardFrontalWiper(PlayerSpaceship *player)
    {
        if (player && !player->IsPendingDestory())
        {
            player->SetShooter(unique<FrontWiper>{new FrontWiper{player, 0.4f, {50.f, 0.f}}});
        }
    }
